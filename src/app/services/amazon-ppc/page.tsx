        ...
        <div className="mx-auto grid max-w-[1440px] gap-14 px-6 lg:grid-cols-[1.05fr_.95fr] lg:items-center lg:px-10">
          ...
          <aside className="rounded-[2rem] border border-white/15 bg-[#081B38] p-6 shadow-2xl shadow-black/30 sm:p-8">
            ...
            <div className="mt-7 grid gap-3 sm:grid-cols-2">
              {[['Demand', 'Capture, defend, and develop the searches that matter.'], ['Products', 'Prioritise the catalogue opportunities worth backing.'], ['Efficiency', 'Put budget behind a defined commercial job.'], ['Context', 'Connect PPC to listings, stock, and conversion.']].map(([title, copy], index) => <div key={title} className="rounded-2xl border border-white/10 bg-white/[.045] p-4"><p className="gm-num-meta" style={{ color: "#FF8A90" }}>0{index + 1}</p><p className="mt-6 font-semibold">{title}</p><p className="mt-2 text-sm leading-6 text-white/55">{copy}</p></div>)}
            </div>
