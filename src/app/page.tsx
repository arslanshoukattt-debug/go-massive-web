  ...
  return (
    <main className="overflow-hidden bg-[#f4f3ef] text-[#020d1f]">
      ...
      <section className="px-5 py-20 sm:px-8 lg:px-12 lg:py-28">
        <Reveal className="mx-auto max-w-[1600px]">
          ...
          <div className="gm-layer-list">
            {operatingLayers.map((layer, index) => <article className="gm-layer" key={layer.number}>
      ...
      <section className="bg-[#020d1f] px-5 py-20 text-white sm:px-8 lg:px-12 lg:py-28">
        <Reveal className="mx-auto max-w-[1600px]">
          ...
          <div className="mt-14 grid gap-px bg-white/20 lg:grid-cols-[1.08fr_.92fr]">
            <article className="gm-case-number"><p className="gm-eyebrow text-white/55">Anonymous outcome / Amazon</p><div><span>−42%</span><p>Total advertising cost of sale</p></div><p className="max-w-sm text-sm leading-6 text-white/60">Following a rebuilt marketplace system for an established outdoor and leisure brand.</p></article>
      ...
      <section className="bg-[#f4f3ef] px-5 py-20 sm:px-8 lg:px-12 lg:py-28">
        <Reveal className="mx-auto max-w-[1600px]"><div className="grid gap-10 lg:grid-cols-[.64fr_1.36fr] lg:items-end"><p className="gm-eyebrow">The Go Massive record</p><h2 className="text-[clamp(3.4rem,6.3vw,6.9rem)] font-semibold uppercase leading-[.8] tracking-[-.1em]">Serious growth needs a team built to stay accountable.</h2></div><div className="mt-14 grid border-l border-t border-[#020d1f]/20 sm:grid-cols-2 lg:grid-cols-4">{agencyStats.map((stat, index) => <div className="gm-stat" key={stat.label}><span>0{index + 1}</span><p>{stat.value}</p><small>{stat.label}</small></div>)}</div></Reveal>
      </section>
