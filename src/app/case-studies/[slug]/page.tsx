...
export default async function CaseStudyDetailPage({ params }: PageProps) {
  ...
  return (
    <main className="bg-[#F7F8FA] text-[#020D1F]">
      ...
      <section className="py-20 sm:py-28"><div className="mx-auto max-w-[1120px] px-6 lg:px-10"><div className="grid gap-10 border-b border-[#020D1F]/10 pb-16 lg:grid-cols-[.7fr_1.3fr]"><p className="text-xs font-bold uppercase tracking-[.18em] text-[#E91A24]">Brand snapshot</p><dl className="grid gap-7 sm:grid-cols-2"><div><dt className="text-xs font-bold uppercase tracking-[.14em] text-[#687385]">Category</dt><dd className="mt-2 font-semibold">{caseStudy.category}</dd></div><div><dt className="text-xs font-bold uppercase tracking-[.14em] text-[#687385]">Marketplace</dt><dd className="mt-2 font-semibold">{caseStudy.marketplace}</dd></div><div><dt className="text-xs font-bold uppercase tracking-[.14em] text-[#687385]">Business stage</dt><dd className="mt-2 font-semibold">{caseStudy.businessStage}</dd></div><div><dt className="text-xs font-bold uppercase tracking-[.14em] text-[#687385]">Go Massive scope</dt><dd className="mt-2 font-semibold">{caseStudy.scope}</dd></div></dl></div>
        <div className="grid gap-10 border-b border-[#020D1F]/10 py-16 lg:grid-cols-[.7fr_1.3fr]"><p className="text-xs font-bold uppercase tracking-[.18em] text-[#E91A24]">The challenge</p><div className="space-y-5 text-lg leading-8 text-[#4E5A6B]">{caseStudy.challenge.map(item=><p key={item}>{item}</p>)}</div></div>
        <div className="grid gap-10 border-b border-[#020D1F]/10 py-16 lg:grid-cols-[.7fr_1.3fr]"><p className="text-xs font-bold uppercase tracking-[.18em] text-[#E91A24]">What Go Massive did</p><ul className="space-y-4">{caseStudy.execution.map(item=><li className="flex gap-4 text-lg leading-8 text-[#4E5A6B]" key={item}><Check size={19} className="mt-1.5 shrink-0 text-[#E91A24]" strokeWidth={3} />{item}</li>)}</ul></div>
        <div className="grid gap-10 py-16 lg:grid-cols-[.7fr_1.3fr]"><p className="text-xs font-bold uppercase tracking-[.18em] text-[#E91A24]">The outcome</p><div className="rounded-3xl bg-white p-8 shadow-sm sm:p-10"><div className="space-y-5 text-lg leading-8 text-[#4E5A6B]">{caseStudy.result.map(item=><p key={item}>{item}</p>)}</div><div className="mt-10 border-t border-[#020D1F]/10 pt-7"><p className="text-sm font-bold uppercase tracking-[.14em] text-[#E91A24]">Takeaway</p><p className="mt-4 text-xl font-semibold leading-8 tracking-tight">{caseStudy.takeaway}</p></div></div></div></div></section>
      <section className="bg-[#E91A24] py-20 text-white"><div className="mx-auto flex max-w-[1120px] flex-col gap-8 px-6 lg:flex-row lg:items-end lg:justify-between lg:px-10"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-white/65">Your opportunity is different</p><h2 className="mt-5 max-w-2xl text-4xl font-semibold tracking-[-.055em]">Start with the facts in your own account.</h2></div><a href="/growth-audit" className="inline-flex shrink-0 items-center gap-2 rounded-full bg-[#020D1F] px-6 py-4 font-semibold transition hover:bg-white hover:text-[#020D1F]">Get a Growth Audit <ArrowRight size={18} /></a></div></section>
      <SiteFooter />
    </main>
  );
}
