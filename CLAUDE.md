...
2. Problem (bg-white): centered red eyebrow "Where brands get stuck" at top → below, description LEFT (right-aligned, muted) | bold headline RIGHT ("Growth rarely breaks in one place[red period]", clamp(2.2–4.2rem), lg vertical hairline between the columns); 6 pain points as icon-led cells on a white hairline lattice (gap-px, border-y edges, no boxes): red lucide icon in E91A24/10 circle + 19px semibold title + muted copy; closing line has a 3px red left bar, semibold first sentence. Iterated 4× with owner (cards → stacked rows → cream lattice → this); his serif reference = inspiration only, site fonts/colors kept, section bg must be white not the page's cream.
