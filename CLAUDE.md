...
1b. **Recognitions & certifications** (slim band under hero, border-b): eyebrow left + 5 hairline tiles in the BrandWall idiom (square, 1px border, hover red) — Amazon Ads / Amazon SPN (Verified Partner), Amazon SAS (Core), Google Ads (Partner), Clutch (5.0-rated agency); name + mono uppercase sublabel. Owner-provided (Aug 2026). NOTE: owner rejected a literal copy of his reference screenshot (rounded notched panel + wordmarks) — screenshots are inspiration, design must stay native to the site's system; The Manifest cut at owner's request.
