...
2. Problem: "Growth rarely breaks in one place[red period]" — headline clamp(1.9–3.4rem) with lg vertical hairline between intro column and headline; 6 pain points as icon-led cells on a hairline lattice (gap-px bg trick on #f4f3ef, border-y edges, no boxes): red lucide icon in E91A24/10 circle + 18px semibold title + muted copy; closing line carries a 3px red left bar with semibold first sentence. Iterated 3× (cards → stacked rows → this, from owner's serif reference used as inspiration only; site fonts/colors kept).
