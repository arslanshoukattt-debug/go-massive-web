...
3. Brands: single-line logo MARQUEE (owner direction Aug 2026, pro-merit.com as reference; replaced cloud, which replaced BrandWall tiles) — centered "Trusted by leading brands worldwide[red period]" + 9 owner-cleared logos auto-scrolling in one continuous seamless loop (.gm-logo-marquee: duplicated track, translateX(-50%) 34s linear, edge mask fade, pause on hover); logos are plain <img> PNGs (transparent bgs — light canvases removed via sharp v2 script; funSLINGER's blue canvas is integral, kept), grayscale/60% opacity → colour on hover, per-logo height classes (h-9…h-14) equalise visual mass across aspect ratios. Logos only — no labels/metrics on named brands.
