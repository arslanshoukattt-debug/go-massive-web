...
3. Brands: TWO counter-scrolling logo marquee rows (owner refs: pro-merit.com then canopymanagement.com; earlier cloud + BrandWall tiles retired) — centered "Trusted by leading brands worldwide[red period]" + 13 owner-cleared logos split 7/6 across rows (`brandLogoRows` in page.tsx); row 1 scrolls left 36s, row 2 right 42s (.gm-logo-track--reverse), each track = 4 identical sets looping translateX(-50%) so rows stay seamless below viewport width; edge mask fade, pause on hover. Logos: plain <img> transparent PNGs (owner supplied removebg versions Aug 2026; his new Qnaturals export was blank — kept prior good file), grayscale/60% → colour on hover, per-logo h-9…h-14 equalises visual mass. To add a logo: process into /public/logos (trim + ≤600px) and append to a row. No labels/metrics on named brands.
